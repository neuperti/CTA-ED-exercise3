...
for (i in seq_along(methods)) {
  ...
  test <- dfmat %>%
    dfm_group(groups = username) %>%
    textstat_dist(margin = "documents", method = sim_method) #specify method here as character object created above
...
```{r}
speeches$fog <- textstat_readability(speeches$text, measure = "FOG")[,2] #number of 3-syllable words
speeches$ari <- textstat_readability(speeches$text, measure = "ARI")[,2] #number characters in words
speeches$drp <- textstat_readability(speeches$text, measure = "DRP")[,2] #degrees of reading power

#get mean and standard deviation of FOG
...
#get mean and standard deviation of ARI
...
#get mean and standard deviation of DRP
